...
namespace VirtualMachine
{
    ...
    unsafe public struct Variant
    {
        ...
        public static Variant operator +(Variant op1, Variant op2)
        {
            ...
            {
                char[] res = new char[op1.m_nValue1 + op2.m_nValue1];
                
                fixed (char* p = res)
                {
                    int size = sizeof(char) * op1.m_nValue1;
                    Buffer.MemoryCopy(op1.m_pValue, p, size, size);
                    size = sizeof(char) * op2.m_nValue1;
                    Buffer.MemoryCopy(op2.m_pValue, (void*)(((char*)p) + op1.m_nValue1), size, size);
                }
